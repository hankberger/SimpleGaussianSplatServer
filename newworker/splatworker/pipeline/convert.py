...
def ply_to_splat(ply_path: Path, output_path: Path) -> Path:
    """Convert a 3DGS PLY file to .splat binary format (32 bytes/gaussian)."""
    ...
    buf[:, 24] = np.clip((0.5 + SH_C0 * f_dc_0) * 255, 0, 255).astype(np.uint8)
    buf[:, 25] = np.clip((0.5 + SH_C0 * f_dc_1) * 255, 0, 255).astype(np.uint8)
    buf[:, 26] = np.clip((0.5 + SH_C0 * f_dc_2) * 255, 0, 255).astype(np.uint8)
    buf[:, 27] = np.clip((1.0 / (1.0 + np.exp(-opacity))) * 255, 0, 255).astype(np.uint8)

    quats = np.stack([rot_0, rot_1, rot_2, rot_3], axis=-1)
    norms = np.maximum(np.linalg.norm(quats, axis=-1, keepdims=True), 1e-10)
    quats_u8 = np.clip(quats / norms * 128 + 128, 0, 255).astype(np.uint8)
    buf[:, 28:32] = quats_u8
